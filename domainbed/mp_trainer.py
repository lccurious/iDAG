...
def main_worker(gpu, ngpus_per_node, 
    ...
    if args.distributed:
        # For multiprocessing distribued, DistributedDataParallel constructor
        ...
        if args.gpu is not None:
            ...
            # available GPUs if devices_ids are not set
            algorithm = torch.nn.parallel.DistributedDataParallel(algorithm)
    elif args.gpu is not None:
        torch.cuda.set_device(args.gpu)
        algorithm = algorithm.cuda(args.gpu)
    ...
    for step in range(n_steps):
        ...
        step_vals = algorithm.module.update(**inputs)
        ...
        if swad:
            ...
            swad_algorithm.update_parameters(algorithm.module, step=step)
        ...
        if args.gpu == 0 and step % args.tb_freq == 0:
